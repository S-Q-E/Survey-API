    ...
    def validate(self, attrs):
        ...
        try:
            if question_type == 'one' or question_type == 'text':
                obj = Answer.objects.get(question=attrs['question'].id, survey=attrs['survey'], user_id=attrs['user_id'], choice=attrs['choice'])
            elif question_type == 'multiple':
                obj = Answer.objects.get(question=attrs['question'].id, survey=attrs['survey'], user_id=attrs['user_id'], choice=attrs['choice'])
        except Answer.DoesNotExist:
            return attrs
        else:
            raise serializers.ValidationError('Уже ответили на вопрос')
...
class QuestionSerializer(serializers.Serializer):
    ...
    question_type = serializers.CharField(max_length=255)
    choices = ChoiceSerializer(many=True, read_only=True)


    def validate(self, attrs):
        question_type = attrs['question_type']
        if question_type == 'one' or question_type == 'mutiple' or question_type == 'text':
            return attrs
        raise serializers.ValidationError('Тип вопроса может быть только с одним варинатомб несколькими или в виде текса')
...
class SurveySerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        if 'start_date' in validated_data:
            raise serializers.ValidationError({'start_date': 'Невозможно изменить это поле'})
        for key, value in validated_data.items():
            setattr(instance, key, value)
        instance.save()
        return instance